...
export default function AttendancePage() {
    ...
    if (loading && history.length === 0) return (
        <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-black/20">Accessing Neural Logs...</p>
        </div>
    );
        ...
        <div className="space-y-12 animate-fade-in pb-20 max-w-[1700px]">
            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10">
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-black/20">Temporal Verification Protocol</span>
                ...
                {user?.role === 'EMPLOYEE' && (
                    <div className="flex items-center gap-6 w-full lg:w-auto">
                        ...
                        <button
                            ...
                            className="flex-1 lg:flex-none flex items-center justify-center gap-4 py-6 px-12 bg-white border border-black/5 text-black font-black text-[11px] uppercase tracking-widest rounded-2xl transition-all active:scale-95"
                        >
                            OUTBOUND
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-white border border-black/5 p-8 rounded-3xl space-y-4">
                    <History className="text-black/20" size={24} />
                    <p className="text-[10px] font-black uppercase tracking-widest text-black/40">Temporal State</p>
                    <p className="text-2xl font-black text-black uppercase tracking-tighter">Active Logs</p>
                </div>
                <div className="bg-white border border-black/5 p-8 rounded-3xl space-y-4">
                    <Clock className="text-black/20" size={24} />
                    <p className="text-[10px] font-black uppercase tracking-widest text-black/40">Sync Status</p>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="text-sm">
                            {history.length === 0 ? (
                                <tr>
                                    <td colSpan={user?.role === 'EMPLOYEE' ? 5 : 6} className="p-32 text-center text-black/10 font-black uppercase tracking-widest text-xs">
                                        No telemetry records found
                                    </td>
                                </tr>
                            ) : (
                                history.map((record: any) => (
                                    <tr key={record.id} className="border-b border-black/5 hover:bg-white/[0.02] transition-colors group">
                                        <td className="px-10 py-8">
                                            <div className="flex items-center gap-6">
                                                ...
                                                <p className="font-black text-black uppercase tracking-tight text-base italic">
                                        ...
                                        {user?.role !== 'EMPLOYEE' && (
                                            <td className="px-10 py-8">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-[10px] font-black text-black/40">
                                                        <User size={14} />
                                                    </div>
                                                    <div>
                                                        <p className="text-[11px] font-black text-black uppercase">{record.user.name}</p>
                                                        <p className="text-[8px] font-bold text-black/20 uppercase tracking-widest">{record.user.department?.name || 'ORPHAN_NODE'}</p>
                                                    </div>
                                                </div>
                                            </td>
                                        )}
                                        <td className="px-10 py-8 text-center tabular-nums font-black text-[11px] text-black/40 group-hover:text-black transition-colors uppercase tracking-widest text-lg italic">
                                        ...
                                        <td className="px-10 py-8 text-right">
                                            <span className="text-xl font-black text-black tracking-tighter italic">
                                                {record.workingHours.toFixed(1)} <span className="text-[10px] text-black/20 uppercase not-italic">HOURS</span>
                                            </span>
                                        </td>
                                    </tr>
                                ))
