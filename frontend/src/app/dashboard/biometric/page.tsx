            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10">
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20">Biometric eSSL Protocol</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-[#111] border border-white/5 p-10 rounded-[2.5rem] flex flex-col justify-between">
                    <div className="space-y-10">
                        <div className="p-5 bg-white/5 text-white/20 rounded-2xl w-fit border border-white/5">
                            <Cpu size={32} />
                        </div>
                        <h3 className="text-2xl font-black uppercase tracking-tight text-white italic">Hardware State</h3>
                        <div className="space-y-6">
                            <div className="flex justify-between items-center border-b border-white/5 pb-4">
                                <span className="text-[10px] uppercase font-black tracking-widest text-white/20">Logic IP</span>
                                <span className="text-[11px] font-black text-white italic">192.168.1.100</span>
                            </div>
                            <div className="flex justify-between items-center border-b border-white/5 pb-4">
                                <span className="text-[10px] uppercase font-black tracking-widest text-white/20">Frequency</span>
                                <span className="text-[11px] font-black text-white italic">CONTINUOUS_POLLING</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-[10px] uppercase font-black tracking-widest text-white/20">Protocol Status</span>
                    ...
                    <div className="mt-12 p-6 bg-white/[0.02] rounded-2xl border border-white/5 flex items-center gap-4">
                        <Activity size={20} className="text-blue-500" />
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-white">Neural Heartbeat</p>
                            <p className="text-[9px] text-white/20 font-bold uppercase tracking-tighter">Syncing logs every 60m...</p>
                        </div>
                    </div>
                </div>

                {/* Logs Table */}
                <div className="bg-[#111] border border-white/5 rounded-[2.5rem] lg:col-span-2 overflow-hidden flex flex-col">
                    <div className="p-10 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                        <h3 className="text-xl font-black uppercase tracking-tight flex items-center gap-4 text-white">
                            <History size={24} className="text-white/20" /> Telemetry Logs
                        </h3>
                        <span className="text-[9px] font-black uppercase tracking-widest text-white/40 italic">Historical Registry</span>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="text-sm">
                                {logs.length === 0 ? (
                                    <tr>
                                        <td colSpan={3} className="p-32 text-center font-black uppercase text-white/10 tracking-[0.4em] text-xs">
                                    ...
                                    logs.map((log: any) => (
                                        <tr key={log.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                                            <td className="px-10 py-8 font-black text-[11px] text-white/60 group-hover:text-white transition-colors italic">
                                            ...
                                            <td className="px-10 py-8 text-right font-black text-white text-lg tracking-tighter italic">
                                                {log.recordsCount} <span className="text-[9px] text-white/10 not-italic uppercase tracking-widest">NODES</span>
                                            </td>
                                        </tr>
                                    ))
                                )}
