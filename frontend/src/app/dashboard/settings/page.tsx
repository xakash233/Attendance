        ...
        <div className="space-y-16 animate-fade-in pb-20 max-w-[1700px] mx-auto">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                ...
                <div className="card bg-white p-12 border-slate-100 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.06)] group hover:scale-[1.01] transition-all relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center mb-14 pb-10 border-b border-gray-50 relative z-10">
                        <div className="flex items-center gap-6">
                            <div className="p-5 bg-slate-50 text-blue-600 rounded-[1.5rem] shadow-inner border border-slate-100 group-hover:bg-blue-600 group-hover:text-white transition-all transform group-hover:rotate-6">
                                <ShieldCheck size={32} strokeWidth={2.5} />
                            </div>
                            <h3 className="text-2xl font-black uppercase tracking-tight text-slate-900">Security Matrix</h3>
                        </div>
                ...
                <div className="card bg-white p-12 border-slate-100 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.06)] group hover:scale-[1.01] transition-all relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center mb-14 pb-10 border-b border-gray-50 relative z-10">
                        <div className="flex items-center gap-6">
                            ...
                            <h3 className="text-2xl font-black uppercase tracking-tight text-slate-900">Knowledge Base</h3>
                        </div>
                        <ActivityIcon size={24} className="text-slate-300 group-hover:text-blue-600 transition-colors animate-pulse" />
                    </div>
                ...
                <div className="card bg-blue-600 p-12 xl:col-span-2 border-none shadow-[0_60px_150px_-30px_rgba(37,99,235,0.2)] relative overflow-hidden group">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 pb-12 border-b border-white/20 relative z-10 gap-8">
                        ...
                        <span className="text-[11px] font-black bg-white/20 text-white px-6 py-2.5 rounded-2xl backdrop-blur-xl border border-white/20 uppercase tracking-[0.3em]">Peripheral Services Active</span>
                    </div>
                    ...
                    <button className="relative z-10 w-full mt-14 py-7 bg-white text-blue-600 rounded-[2rem] font-black uppercase tracking-[0.4em] text-xs shadow-3xl hover:bg-slate-50 transition-all flex items-center justify-center gap-4 active:scale-95 group/btn overflow-hidden">
...
const SettingItem = ({ title, desc, value, icon: Icon }: any) => (
    <div className="flex items-center justify-between p-7 bg-slate-50/50 hover:bg-white rounded-[1.5rem] border border-transparent hover:border-blue-600/20 hover:shadow-2xl hover:shadow-blue-600/5 transition-all cursor-pointer group/item">
        <div className="flex gap-6 items-center">
            <div className="p-4 bg-white text-slate-400 rounded-2xl group-hover/item:bg-blue-600 group-hover/item:text-white transition-all shadow-sm border border-gray-50 group-hover/item:border-blue-500 shadow-inner">
                <Icon size={24} strokeWidth={2.5} />
            </div>
            <div>
                <p className="text-[11px] font-black uppercase tracking-[0.1em] text-slate-900 leading-none mb-2">{title}</p>
                <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400">{desc}</p>
            </div>
        </div>
        <div className="flex items-center gap-6">
            ...
            <ChevronRight size={18} className="text-slate-200 opacity-0 group-hover/item:opacity-100 group-hover/item:translate-x-2 transition-all" />
        </div>
    </div>
);

const ChannelCard = ({ name, status, desc, icon: Icon }: any) => (
    <div className="p-10 bg-white/10 rounded-[2.5rem] border border-white/10 group/card hover:bg-white/20 transition-all backdrop-blur-sm">
        <div className="w-16 h-16 bg-white/10 rounded-[1.5rem] mb-8 flex items-center justify-center text-white shadow-inner group-hover/card:scale-110 group-hover/card:bg-white group-hover/card:text-blue-600 transition-all duration-500">
            <Icon size={32} strokeWidth={2.5} />
        </div>
        <p className="text-xl font-black uppercase tracking-tight text-white mb-3 leading-none">{name}</p>
        <p className="text-[10px] font-bold text-white/60 leading-relaxed mb-8 uppercase tracking-[0.1em]">{desc}</p>
        <div className="flex items-center gap-3 pt-6 border-t border-white/10">
            <div className="w-2 h-2 rounded-full bg-white animate-pulse shadow-[0_0_10px_#fff]"></div>
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/90">SIGNAL: {status.toUpperCase()}</p>
        </div>
    </div>
);
